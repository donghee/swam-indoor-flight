...
class MavrosCommon:
    def __init__(self):
        ...
        self.sub_topics_ready = {
            key: False
            for key in [
                    'alt', 'ext_state', 'local_pos', 'mission_wp', 'state'
            ]
        }
    ...
    def wait_for_topics(self, timeout):
        ...
        for i in xrange(timeout * loop_freq):
            rospy.loginfo(self.sub_topics_ready)
    ...
    def wait_for_landed_state(self, desired_landed_state, timeout, index):
        ...
        if landed_state_confirmed == False:
            rospy.loginfo(
                "landed state not detected | desired: {0}, current: {1} | index: {2}, timeout(seconds): {3}".
                format(mavutil.mavlink.enums['MAV_LANDED_STATE'][desired_landed_state].name, mavutil.mavlink.enums[
                            'MAV_LANDED_STATE'][self.extended_state.landed_state].name, index, timeout))
        ...
        if transitioned == False:
            rospy.loginfo(
                ...
                format(mavutil.mavlink.enums['MAV_VTOL_STATE'][transition].name,
                       mavutil.mavlink.enums['MAV_VTOL_STATE'][self.extended_state.vtol_state].name, index, timeout))
